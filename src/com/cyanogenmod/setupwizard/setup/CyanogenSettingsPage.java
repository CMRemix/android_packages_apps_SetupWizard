...
public class CyanogenSettingsPage extends SetupPage {
    ...
    public void onFinishSetup() {
        getCallbacks().addFinishRunnable(new Runnable() {
            @Override
            public void run() {
                if (getData().containsKey(NAVIGATION_BAR_SHOW)) {
                    SetupStats.addEvent(SetupStats.Categories.SETTING_CHANGED,
                            SetupStats.Action.ENABLE_NAV_KEYS,
                            SetupStats.Label.CHECKED,
                            String.valueOf(getData().getBoolean(NAVIGATION_BAR_SHOW)));
                    writeDisableNavkeysOption(mContext, getData().getBoolean(NAVIGATION_BAR_SHOW));
                }
            }
        });
        handleEnableMetrics();
    ...
    public static class CyanogenSettingsFragment extends SetupPageFragment {
        ...
        private View.OnClickListener mNavKeysClickListener = new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                mPage.getData().putBoolean(NAVIGATION_BAR_SHOW, checked);
            }
        };
        ...
        private void updateDisableNavkeysOption() {
            if (!mHideNavKeysRow) {
                ...
                boolean checked = myPageBundle.containsKey(NAVIGATION_BAR_SHOW) ?
                        myPageBundle.getBoolean(NAVIGATION_BAR_SHOW) :
                        enabled;
                mNavKeys.setChecked(checked);
                myPageBundle.putBoolean(NAVIGATION_BAR_SHOW, checked);
            }
        }
